...
class SoundSynth {
   ...
   *   "OH" -> F1: 570Hz,  F3: 840Hz,  F3: 2410Hz
  ...
  private playFormantSpeech(
    ...
    try {
      ...
      const vol = (options.volumeMultiplier ?? 1.0) * 0.35;

      // 1. Core Source: Heavy combination of Sawtooth and Triangle for raw vocal buzz
      const oscSaw = ctx.createOscillator();
      const oscTri = ctx.createOscillator();
      
      oscSaw.type = options.raspy ? "sawtooth" : "sawtooth";
      oscTri.type = "sawtooth"; // Gives extra rich overtones

      oscSaw.frequency.setValueAtTime(startPitch, now);
      oscSaw.frequency.exponentialRampToValueAtTime(endPitch, now + duration);
      
      oscTri.frequency.setValueAtTime(startPitch * 1.01, now); // slightly detuned for chorus thickness
      oscTri.frequency.exponentialRampToValueAtTime(endPitch * 1.01, now + duration);

      // Low frequency pitch vibrato for standard vocal expression
      if (options.vibrato) {
        const lfo = ctx.createOscillator();
        lfo.frequency.setValueAtTime(6.5, now); // ~6.5 Hz vibrato
        const lfoGain = ctx.createGain();
        lfoGain.gain.setValueAtTime(8, now); // Pitch wobble depth
        lfo.connect(lfoGain);
      ...
      voiceFilter.frequency.setValueAtTime(450, now);
      ...
      mainGain.gain.linearRampToValueAtTime(vol, now + 0.08); // fade-in
      ...
      f2Gain.gain.setValueAtTime(0.6, now); // F2 is quieter
      f3Gain.gain.setValueAtTime(0.35, now); // F3 is quietest
      ...
      if (options.shStart) {
        ...
        shFilter.frequency.setValueAtTime(3500, now);

        const shGain = ctx.createGain();
        shGain.gain.setValueAtTime(0.08, now);
      ...
      if (options.tEnd) {
        ...
        clickNoise.frequency.setValueAtTime(4000, now + duration - 0.04);
        const clickGain = ctx.createGain();
        clickGain.gain.setValueAtTime(0.001, now);
        clickGain.gain.setValueAtTime(0.12, now + duration - 0.04);
  ...
  /* ========================================================================
     Vocal Phonovox Expressions (Stylized with deep, swagger-filled voice formants)
     ======================================================================== */

  /**
   * Spoken Vowel Accent: "BRUHHH!"
   * Rich, deep, bass-y, velvet-toned baritone slide, with short nasal transient.
   * Vowel used: "UH" (F1: 520, F2: 1190, F3: 2390)
   */
  speakBruh() {
    // 85Hz sliding down to 68Hz - legendary deep voice pitch
    this.playFormantSpeech(85, 68, 0.72, {
      f1: 510,
      f2: 1150,
      f3: 2300,
    }, {
   ...
   * High-energy, long vocal tone.
   * Begins with an airy "SH" noise, then transitions into a resonating "EE" vowel.
   * Pitch stays steady around 160Hz with high vibrato.
   */
  speakSheeeesh() {
    this.playFormantSpeech(165, 172, 1.1, {
      f1: 270,
      f2: 2290,
      f3: 3010
    }, {
   ...
   * Two rapid consecutive deep vowel segments.
  ...
  speakNaw() {
    ...
    try {
      ...
      // "AH"
      this.playFormantSpeech(95, 80, 0.35, { f1: 730, f2: 1090, f3: 2440 }, { nasal: true });
      
      // Delay second word "OH"
      setTimeout(() => {
        this.playFormantSpeech(78, 62, 0.5, { f1: 570, f2: 840, f3: 2410 }, { vibrato: true, nasal: true });
      }, 340);
    } catch (e) {
       console.error(e);
    }
  ...
  /**
   * Spoken Vowel Accent: "WHAT?!" (Shocked, high inflection rise at end)
   * Vowel used: "UH" -> slides up in frequency at the end with a sharp "T" cutoff.
   */
  speakWhat() {
    this.playFormantSpeech(110, 155, 0.32, {
      f1: 520,
      f2: 1250,
      f3: 2400
    }, {
   ...
   * Segment 1: Confident "NO" (110Hz to 95Hz)
   * Segment 2: Deep "RUG" (vowel "UH" at 85Hz) with a fast start.
   */
  speakNoRug() {
    if (this.isMuted) return;
    try {
      this.playFormantSpeech(115, 95, 0.28, { f1: 570, f2: 840, f3: 2410 });
      setTimeout(() => {
        this.playFormantSpeech(92, 75, 0.42, { f1: 520, f2: 1190, f3: 2390 }, { raspy: true, nasal: true });
      }, 260);
    } catch (e) {
      console.error(e);
    }
  ...
  speakGetRekt() {
    if (this.isMuted) return;
    try {
      this.playFormantSpeech(125, 120, 0.2, { f1: 530, f2: 1840, f3: 2480 }, { tEnd: true });
      
      setTimeout(() => {
        this.playFormantSpeech(135, 125, 0.22, { f1: 530, f2: 1840, f3: 2480 }, { raspy: true, tEnd: true });
      }, 220);

      setTimeout(() => {
        this.playFormantSpeech(110, 80, 0.38, { f1: 520, f2: 1190, f3: 2390 }, { raspy: true, vibrato: true });
      }, 440);
    } catch (e) {
      console.error(e);
    }
   ...
   * Rapid reminder: "RENT DUE" -> "BOYYY" sliding extremely low.
   */
  speakRentDue() {
    if (this.isMuted) return;
    try {
      this.playFormantSpeech(105, 100, 0.25, { f1: 530, f2: 1840, f3: 2480 });
      setTimeout(() => {
        this.playFormantSpeech(95, 90, 0.22, { f1: 270, f2: 2290, f3: 3010 });
      }, 240);
      setTimeout(() => {
        this.playFormantSpeech(110, 60, 0.7, { f1: 570, f2: 840, f3: 2410 }, { vibrato: true, nasal: true, volumeMultiplier: 1.3 });
      }, 460);
    } catch (e) {
      console.error(e);
    }
   ...
   * Wojak Skeleton scream. High, crispy, rasped squeal.
   */
  speakSellIt() {
    if (this.isMuted) return;
    try {
      this.playFormantSpeech(210, 240, 0.3, { f1: 530, f2: 1840, f3: 2480 }, { raspy: true });
      setTimeout(() => {
        this.playFormantSpeech(240, 180, 0.35, { f1: 270, f2: 2290, f3: 3010 }, { raspy: true, tEnd: true });
      }, 280);
    } catch (e) {
      console.error(e);
    }
