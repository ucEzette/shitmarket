...
while true; do
  ...
  if echo "$output" | grep -q "feature \`edition2024\` is required"; then
    ...
    prev_ver=$(curl -s "https://crates.io/api/v1/crates/$pkg_name" | grep -o '"num":"[^"]*"' | head -n 20 | grep -o '"[^"]*"$' | tr -d '"' | grep -v 'alpha\|beta\|rc' | awk "/^$(echo $pkg_full | sed "s/$pkg_name-//")$/{getline; print; exit}")
    
    if [ -z "$prev_ver" ]; then
        echo "Could not find previous version for $pkg_name!"
        exit 1
    fi
    echo "Downgrading $pkg_name to $prev_ver due to edition2024"
    cargo update -p "$pkg_name@$current_ver" --precise "$prev_ver"
