...
function scalePriceTo1e8(priceUsdStr: string | undefined, defaultPrice: bigint): bigint {
  if (!priceUsdStr) return defaultPrice;
  const val = parseFloat(priceUsdStr);
  if (!isFinite(val) || val <= 0) return defaultPrice;
